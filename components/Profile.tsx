...
function Profile({ balance, onTopUp }: { balance: number; onTopUp: () => void }) {
...
const menuSections = [
    ...
    title: "Preferences",
    items: [
        { icon: "🔔", label: "Push Notifications", toggle: true, value: notificationsOn, onToggle: () => setNotificationsOn((v) => !v) },
        { icon: "🔑", label: "Biometric Login", toggle: true, value: biometricsOn, onToggle: () => setBiometricsOn((v) => !v) },
...
return (
    ...
    <div
        ...
        style={{ background: "linear-gradient(160deg, #FF9800 0%, #FF5722 100%)" }}
    >
        ...
        <div>
            ...
            <p className="text-white/60 text-xs mt-0.5">Member since Jan 2024 · Gold tier</p>
        </div>
        </div>
    </div>
    ...
    <div className="px-5 space-y-5">
        ...
        <p className="text-center text-xs text-muted-foreground pb-2">ChowWallet v2.1.0</p>
    </div>
    </div>
);
}
